...
export default function HeroButton({ label1, label2, onClick, colors = "purple" }) {
  const l1 = label1.split("");
  const l2 = label2.split("");
        ...
        <div className="wrap">
          ...
          <div className="content">
            <span className="char state-1">
              {l1.map((ch, i) => (
                <span key={i} data-label={ch} style={{ "--i": i + 1 }}>{ch}</span>
              ))}
            </span>
...
const StyledWrapper = styled.div`
  .button {
    --white:      ${({ $colors }) => THEMES[$colors]?.white   || THEMES.purple.white};
    --c100:       ${({ $colors }) => THEMES[$colors]?.c100    || THEMES.purple.c100};
    --c200:       ${({ $colors }) => THEMES[$colors]?.c200    || THEMES.purple.c200};
    --c300:       ${({ $colors }) => THEMES[$colors]?.c300    || THEMES.purple.c300};
    --c400:       ${({ $colors }) => THEMES[$colors]?.c400    || THEMES.purple.c400};
    --c500:       ${({ $colors }) => THEMES[$colors]?.c500    || THEMES.purple.c500};
    --shadow-rgb: ${({ $colors }) => THEMES[$colors]?.shadow  || THEMES.purple.shadow};
    --radius: 18px;
  ...
  .bg::before {
    ...
    box-shadow:
      -7px 6px 0 0 rgb(var(--shadow-rgb) / 40%),
      -14px 12px 0 0 rgb(var(--shadow-rgb) / 30%),
      -21px 18px 4px 0 rgb(var(--shadow-rgb) / 25%),
      -28px 24px 8px 0 rgb(var(--shadow-rgb) / 15%),
      -35px 30px 12px 0 rgb(var(--shadow-rgb) / 12%),
      -42px 36px 16px 0 rgb(var(--shadow-rgb) / 8%),
      -56px 42px 20px 0 rgb(var(--shadow-rgb) / 5%);
  }
  ...
  .wrap {
    ...
    padding: 3px;
    background: linear-gradient(to bottom, var(--c100) 0%, var(--c400) 100%);
    position: relative;
    transition: all 0.3s ease;
  }
  ...
  .content {
    ...
    font-weight: 600;
    transition: all 0.3s ease;
    background: linear-gradient(to bottom, var(--c300) 0%, var(--c400) 100%);
    box-shadow:
      inset -2px 12px 11px -5px var(--c200),
  ...
  .content::before {
    ...
    opacity: 0.7;
    margin: auto;
    background: linear-gradient(to bottom, transparent, var(--c400));
  ...
  .char span::after {
    ...
    position: absolute;
    color: var(--white);
    text-shadow: -1px 1px 2px var(--c500);
    left: 0;
  }
  .char span::before { opacity: 0; transform: translateY(-100%); }
  .char.state-2 { position: absolute; left: 70px; }
  ...
  .icon div {
    position: relative;
    width: 24px;
    box-shadow: -2px 2px 5px var(--c400);
    transform: scale(0.9);
    background: linear-gradient(to bottom, var(--white), var(--c100));
  ...
  .icon div::before {
    ...
    top: 1px;
    box-shadow: 1px -2px 3px -1px var(--c400);
  ...
  .icon div::after {
    ...
    box-shadow: -2px 2px 3px 0 var(--c400);
    background: linear-gradient(200deg, var(--white), var(--c100));
  ...
  .button:active .bg::before {
    ...
    box-shadow:
      -7px 6px 0 0 rgb(var(--shadow-rgb) / 40%),
      -14px 12px 0 0 rgb(var(--shadow-rgb) / 25%),
      -21px 18px 4px 0 rgb(var(--shadow-rgb) / 15%);
  }
  .button:active .content {
    box-shadow:
      inset -1px 12px 8px -5px rgba(0,0,0,0.4),
      inset 0px -3px 8px 0px var(--c200);
  }
