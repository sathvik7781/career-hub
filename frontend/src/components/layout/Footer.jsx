  ...
  return (
    <CubeWrapper>
      <div className="cube-container">
        {[0, 1, 2].map((ci) => (
          <div key={ci} className={`cube cube-${ci}`}>
            {[-1, 0, 1].map((x, di) => (
              <div key={di} className="col" style={{ "--x": x, "--y": 0 }}>
                {[3, 2, 1].map((i) => (
    ...
    <FooterWrapper>
      ...
      <div className="footer-bottom">
        <p className="footer-copy">
          © 2026 <span className="footer-brand">CareerHub</span>. All rights reserved.
        </p>
        <SocialButtons>
          <div className="card">
        ...
        <p className="footer-credit">
          Crafted by <span className="footer-author">Sathvik</span>
        </p>
      </div>
    </FooterWrapper>
  );
...
const CubeWrapper = styled.div`
  ...
  /* Fixed size so the absolute children have a reference point */
  width: 200px;
  height: 200px;
  position: relative;
  ...
  /* Each cube is a 50x50 anchor at the center */
  .cube {
    position: absolute;
    top: 50%;
    left: 50%;
  ...
  .cube-0 { z-index: 2; transform: translate(-50%, -50%); }
  .cube-1 { z-index: 1; transform: translate(calc(-50% - 60px), calc(-50% - 60px)); }
  .cube-2 { z-index: 3; transform: translate(calc(-50% + 60px), calc(-50% + 60px)); }

  /* Each column of 3 spans, offset by --x */
  .col {
    position: absolute;
    top: 0;
    left: 0;
  ...
  .col span {
    ...
    transition: background 1.5s, filter 1.5s;
    cursor: pointer;
  }
  ...
  .col span:hover::before,
  .col span:hover::after {
    transition: 0s;
    background: #38bdf8;
  }
`;
...
const FooterWrapper = styled.footer`
  ...
  .dark & .footer-links { border-color: rgba(255,255,255,0.07); }
  ...
  .footer-link:hover { color: #0060c4; }
  ...
  .cube-section {
    ...
    overflow: hidden;
    padding-bottom: 5rem;
    background: linear-gradient(to bottom, transparent, rgba(0,96,196,0.05));
  }
  .dark & .cube-section {
  ...
  .dark & .footer-bottom { background: #0f172a; }
  ...
  .footer-author { color: rgba(255,255,255,0.9); font-weight: 600; }
`;
